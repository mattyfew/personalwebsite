...
module.exports = function IndexModule(pb) {
    ...
    /**
     * Index The home page controller of the mattyfew theme.
     * @class Index
    ...
    Index.prototype.render = function(cb) {
        ...
        TopMenu.getTopMenu(self.session, self.ls, options, function(themeSettings, navigation, accountButtons) {
            TopMenu.getBootstrapNav(navigation, accountButtons, function(navigation, accountButtons) {
                ...
                pluginService.getSettings('mattyfew', function(err, mattyfewSettings) {
                    var homePageKeywords = '';
                    var homePageDescription = '';
                    for(var i = 0; i < mattyfewSettings.length; i++) {
                        switch(mattyfewSettings[i].name) {
                            case 'home_page_keywords':
                                homePageKeywords = mattyfewSettings[i].value;
                                break;
                            case 'home_page_description':
                                homePageDescription = mattyfewSettings[i].value;
                                break;
                            default:
                                break;
                        }
                    ...
                    self.ts.load('landing_page', function(err, template) {
                        ...
                        var dao = new pb.DAO();
                        dao.q('mattyfew_theme_settings', opts, function(err, settings) {
                            if (util.isError(err)) {
